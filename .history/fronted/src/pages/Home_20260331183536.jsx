...
<section className="poetry-section container mt-5 pb-5">
  ...
  <img
    /* डेटाबेस (item.img) से इमेज उठाएगा और 'covers' फोल्डर में ढूंढेगा */
    src={item.img ? `/covers/${item.img}` : ""} 
    alt={item.title}
    className="img-fluid"
    loading="lazy"
    onError={(e) => {
      e.target.onerror = null; // ब्लिंकिंग रोकने के लिए
      e.target.style.display = 'none'; // अगर इमेज न मिले तो उसे छुपा दो, ब्लिंक मत करो
    }}
  />
</div>
  ...
  <div className="container px-md-5 px-3">
          ...
          >
            <div className="row g-0">
              <div className="col-md-6 overflow-hidden featured-img-box">
                <img
                  /* हमने यहाँ पाथ को 'covers' रखा है */
                  src={featuredStory.img ? `/covers/${featuredStory.img.trim()}` : ""}
                  className="img-fluid featured-img"
                  alt={featuredStory.title}
                  onError={(e) => {
                    e.target.onerror = null; // लूप को यहीं तोड़ दो
                    e.target.style.display = 'none'; // फोटो नहीं मिली तो गायब कर दो, ब्लिंक मत करो
                  }}
                />
              </div>
    ...
    <div className="slider-wrapper position-relative">
      <button
        ...
        onClick={() => document.getElementById("kahaniSlider").scrollBy({ left: -400, behavior: "smooth" })}
      >
        ...
        {sliderStories.map((story) => (
          ...
          >
            <div
              ...
              style={{ cursor: "pointer" }}
              onClick={() => navigate(`/read/${story.authorId}/${story.title}`)}
            >
              <div className="small-card-img" style={{ background: '#f8f9fa', minHeight: '200px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <img
                  src={story.img ? `/covers/${story.img.trim()}` : ""}
                  className="card-img-top"
                  alt={story.title}
                  onError={(e) => {
                    e.target.onerror = null; 
                    e.target.style.display = 'none'; // ब्लिंकिंग रोकने का पक्का इलाज
                  }}
                />
                {/* अगर इमेज गायब हो जाए, तो बैकग्राउंड में टाइटल दिखेगा */}
                {!story.img && <span className="text-muted p-2 text-center">{story.title}</span>}
              </div>
