...
app.get("/api/content/all/search", async (req, res) => {
  try {
    ...
    if (!query) return res.json([]); // अगर क्वेरी खाली है
    ...
    const [works, authors] = await Promise.all([
      Content.find({
        $or: [
          ...
          { searchTags: { $regex: searchRegex } }, // ✅ कंटेंट के टैग्स में भी ढूंढेगा
        ],
      })
        .limit(20)
        .lean(),
      Author.find({ 
        $or: [
          ...
          { searchTags: { $regex: searchRegex } } // ✅ लेखक के टैग्स में भी ढूंढेगा
        ] 
      })
        .limit(10)
        .lean(),
