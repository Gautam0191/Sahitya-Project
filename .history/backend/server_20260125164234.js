...
const syncFavorites = async () => {
  try {
    ...
    // पहले सबका isFavorite हटाएँ
    await Author.updateMany({}, { $set: { isFavorite: false } });

    // फिर लिस्ट वाले नामों को true करें
    const result = await Author.updateMany(
      { name: { $in: favoriteNames } },
      { $set: { isFavorite: true } }
    );

    console.log(`✅ Favorites Updated: ${result.modifiedCount} authors are now favorites.`);
  } catch (err) {
...
app.get("/api/content/all/search", async (req, res) => {
  try {
    ...
    const [works, authors] = await Promise.all([
      Content.find({
        $or: [{ title: { $regex: searchRegex } }, { authorName: { $regex: searchRegex } }],
      }).limit(20).lean(),
      Author.find({ name: { $regex: searchRegex } }).limit(10).lean(),
    ]);
...
app.get("/api/content/all/:type", async (req, res) => {
  try {
    ...
    const mapping = {
      ...
      nibandh: "essay", sher: "shayari",
      shayari: "shayari", dohe: "doha"
    };
...
app.get("/api/content/:authorId/:title", async (req, res) => {
  try {
    ...
    res.json(work);
  } catch (err) { res.status(500).json({ error: "सर्वर त्रुटि" }); }
});
