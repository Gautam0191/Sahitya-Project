...
app.get("/api/home/drama", async (req, res) => {
  try {
    // 1. पहले वो नाटक ढूँढो जो Featured हैं
    let drama = await Content.find({ type: "drama", isFeatured: true }).limit(5);

    // 2. अगर Featured नाटक कम मिले या नहीं मिले, तो बाकी नाटक ले आओ (ताकि सेक्शन खाली न रहे)
    if (drama.length < 4) {
      const extraDrama = await Content.find({ 
        type: "drama", 
        isFeatured: { $ne: true } // जो पहले से नहीं लिए गए
      }).limit(4 - drama.length);
      
      drama = [...drama, ...extraDrama];
    }
