...
function App() {
  ...
  const handleAuthorScroll = useCallback(() => {
    if (authorScrollRef.current) {
      ...
      // 'prev' का इस्तेमाल करने से स्टेट तभी अपडेट होगी जब ज़रूरत हो
      setShowPrevAuthor((prev) => {
        const nextVal = scrollLeft > 10;
        return prev !== nextVal ? nextVal : prev;
      });

      setShowNextAuthor((prev) => {
        const nextVal = scrollLeft < scrollWidth - clientWidth - 25;
        return prev !== nextVal ? nextVal : prev;
      });
    }
  }, []); // [] का मतलब है कि यह फंक्शन पूरी लाइफसाइकिल में एक ही रहेगा

  // ✅ 2. इसे भी useCallback में डालना बेहतर है
  const scrollAuthor = useCallback((direction) => {
