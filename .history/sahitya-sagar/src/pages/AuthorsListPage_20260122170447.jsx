...
const AuthorsListPage = ({ title, subtitle, categoryFilter }) => {
  ...
  const hindiLetters = ["सभी", "अ", "आ", "इ", "ई", "उ", "ऊ", "ए", "ऐ", "ओ", "औ", "क", "ख", "ग", "घ", "च", "छ", "ज", "झ", "ट", "ठ", "ड", "ढ", "त", "थ", "द", "ध", "न", "प", "फ", "ब", "भ", "म", "य", "र", "ल", "व", "श", "ष", "स", "ह"];

  const displayAuthors = [...authors].sort((a, b) => a.name.localeCompare(b.name, "hi"));
  ...
  if (loading) {
    return (
      <div className="loading-container">
        ...
        <h3 className="mt-3">साहित्यकारों की जानकारी जुटाई जा रही है... 📜</h3>
      </div>
    );
  }
              ...
              <div className="col-6 col-md-4 col-lg-3" key={author._id}>
                <Link to={`/author/${author.id}`} className="text-decoration-none">
                  <div className="author-card text-center p-3 bg-white shadow-sm rounded-4 h-100">
                    ...
                    <p className="author-category">{author.categoryLabel || "साहित्यकार"}</p>
                  </div>
                </Link>
              </div>
            ))
