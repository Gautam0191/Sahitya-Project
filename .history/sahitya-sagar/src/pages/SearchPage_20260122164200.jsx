    ...
    <div className="search-card-premium">
      ...
      <div className="p-4">
        <div className="d-flex justify-content-between align-items-center mb-3">
          <span className={`category-tag ${item.isAuthor ? "tag-author" : "tag-work"}`}>
            <i className={item.isAuthor ? "fas fa-user-nib" : "fas fa-book-open"}></i>
        ...
        <div className="mt-4 pt-3 border-top">
          {item.isAuthor ? (
            <Link to={`/author/${item.id}`} className="btn-royal w-100">
              प्रोफाइल देखें <i className="fas fa-arrow-right ms-2"></i>
            </Link>
          ) : (
            <Link to={`/read/${item.authorId}/${item.title}`} className="btn-royal-outline w-100">
