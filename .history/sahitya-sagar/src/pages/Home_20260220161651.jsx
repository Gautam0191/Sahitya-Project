...
const Home = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 1. कहानी का लॉजिक (जैसा था वैसा ही)
        let picked = null;
        let filtered = [];
        ...
        // 2. कविता का सॉर्टिंग लॉजिक (डिज़ाइन ग्रिड ही रहेगा)
        let sortedPoetry = [];
        if (poetryRes.data) {
  ...
  return (
    <>
      ...
      <section className="poetry-section container mt-5 pb-5">
              ...
              >
                <div className="poetry-card-v2 shadow-sm h-100">
                  <div className="card-img-wrapper">
                    <img
                      /* इमेज पाथ फिक्स: अगर /covers/ से शुरू है तो पोर्ट जोड़ो */
                      src={item.img ? (item.img.startsWith('/') ? `http://localhost:5000${item.img}` : item.img) : "/placeholder.jpg"}
                      alt={item.title}
                      className="img-fluid"
                      loading="lazy"
                      onError={(e) => { 
        ...
        {dbPoetry.length > (isMobile ? 6 : 10) && (
          <div className="text-center mt-5">
            <button className="premium-btn" onClick={() => setShowAll(!showAll)}>
      ...
      {/* --- इसके नीचे का कहानी सेक्शन (कथा कलश) जैसा था वैसा ही रहने दें --- */}
    </>
  );
};
