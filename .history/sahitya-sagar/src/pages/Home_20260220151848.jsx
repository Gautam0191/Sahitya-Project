...
const Home = ({
  ...
  const [mainPoetry, setMainPoetry] = useState(null); // ✅ नई स्टेट: चुनिंदा कविता के लिए
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 2. कहानी का लॉजिक (Featured Story)
        ...
        if (storyRes.data && storyRes.data.length > 0) {
          pickedStory =
            ...
            storyRes.data[0];
          filteredStories = storyRes.data
            .filter((story) => story._id !== pickedStory?._id)
            .slice(0, 6);
        }

        // --- ✅ 3. कविता का नया लॉजिक (Poetry Logic) ---
        let pickedPoetry = null;
        let filteredPoetry = [];
        if (poetryRes.data && poetryRes.data.length > 0) {
          // जो कविता isFeatured: true है उसे उठाओ, नहीं तो पहली वाली ले लो
          pickedPoetry = 
            poetryRes.data.find((p) => p.isFeatured === true) || 
            poetryRes.data[0];
          
          // बाकी कविताएं (featured वाली को हटाकर)
          filteredPoetry = poetryRes.data
            .filter((p) => p._id !== pickedPoetry?._id)
            .slice(0, 10); // जितनी आपको लिस्ट में दिखानी हैं
        }

        // 4. स्टेट्स को एक साथ अपडेट करें
        setMainPoetry(pickedPoetry);   // ✅ चुनिंदा कविता सेट की
        setDbPoetry(filteredPoetry);   // ✅ बाकी कविताओं की लिस्ट सेट की
