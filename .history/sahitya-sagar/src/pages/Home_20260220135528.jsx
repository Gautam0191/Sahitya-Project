...
const Home = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const poetryRes = await axios.get("http://localhost:5000/api/home/poetry");
        setDbPoetry(poetryRes.data);

        const storyRes = await axios.get("http://localhost:5000/api/home/stories");
        if (storyRes.data && storyRes.data.length > 0) {
          setDbStories(storyRes.data);

          // Featured Story चुनें
          const picked = storyRes.data.find((s) => s.isFeatured === true) || storyRes.data[0];
        ...
        const dramaRes = await axios.get("http://localhost:5000/api/home/drama");
        setDbDrama(dramaRes.data);

        const favRes = await axios.get("http://localhost:5000/api/authors/favorites");
