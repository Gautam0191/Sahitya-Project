...
const AuthorsListPage = ({ title, subtitle, categoryFilter }) => { // 'data' हटा दिया क्योंकि अब हम API से लाएंगे
  ...
  useEffect(() => {
    ...
    const fetchAuthors = async () => {
      try {
       const response = await axios.get(`http://localhost:5000/api/authors?category=${categoryFilter}`);
setAuthors(response.data);
        // अगर categoryFilter दिया है (जैसे 'kavi'), तो सिर्फ वही दिखाओ
        if (categoryFilter) {
          const filtered = response.data.filter(auth => auth.category === categoryFilter);
          setAuthors(filtered);
        } else {
          setAuthors(response.data);
        }
  ...
  if (loading) {
    return <div className="text-center py-5"><h3>साहित्यकारों की जानकारी जुटाई जा रही है... 📜</h3></div>;
  }
        ...
        <div className="row g-4 justify-content-center">
          {filteredAuthors.length > 0 ? (
            filteredAuthors.map((author) => (
              <div className="col-6 col-md-4 col-lg-3" key={author._id}> {/* author.id की जगह author._id */}
            ...
            <div className="text-center py-5 w-100">
              ...
              <h4 className="text-muted">अक्षर "{selectedLetter}" से अभी कोई साहित्यकार नहीं मिला।</h4>
