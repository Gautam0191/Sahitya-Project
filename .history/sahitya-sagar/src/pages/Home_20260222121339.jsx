    ...
    const fetchData = async () => {
      try {
        ...
        if (storyRes.data && storyRes.data.length > 0) {
          picked = storyRes.data.find((s) => s.isFeatured === true) || storyRes.data[0];
          filtered = storyRes.data.filter((s) => s._id !== picked?._id).slice(0, 6);
        }

        // 2. कविता का सॉर्टिंग लॉजिक
        let sortedPoetry = [];
        if (poetryRes.data) {
          sortedPoetry = [...poetryRes.data].sort((a, b) => (b.isFeatured === true ? 1 : -1));
        }

        // 3. ✅ नाट्य मंच (Drama) का सॉर्टिंग लॉजिक
        let sortedDrama = [];
        if (dramaRes.data) {
          sortedDrama = [...dramaRes.data].sort((a, b) => (b.isFeatured === true ? 1 : -1));
        }
...
>
  ...
  <div className="carousel-inner">
    ...
    <div className="carousel-item">
      <img src="/path-to-image2.jpg" className="d-block w-100" alt="बैनर 2" style={{ height: isMobile ? "220px" : "450px" }} />
    </div>
    <div className="carousel-item">
      <img src="/path-to-image3.jpg" className="d-block w-100" alt="बैनर 3" style={{ height: isMobile ? "220px" : "450px" }} />
    </div>
    <div className="carousel-item">
      <img src="/path-to-image4.jpg" className="d-block w-100" alt="बैनर 4" style={{ height: isMobile ? "220px" : "450px" }} />
    </div>
  </div>
  ...
  <button className="carousel-control-next" type="button" data-bs-target="#heroCarousel" data-bs-slide="next">
    ...
    <span className="visually-hidden">Next</span>
      ...
      {/* --- 2. कविता सेक्शन (वापस आपके ओरिजिनल डिज़ाइन पर) --- */}
              ...
              >
                <div className="poetry-card-v2 shadow-sm h-100">
                  <div className="card-img-wrapper">
                    <img
                      /* ✅ Bilkul Reading page ki tarah seedha path use karein */
      ...
      <section className="natak-section-wrapper">
        <div className="container py-5">
          <div className="text-center mb-5">
            <h2 className="display-5 fw-bold" style={{ color: "#5a2a3a" }}>नाट्य मंच</h2>
            <div className="title-underline mx-auto"></div>
          </div>
          <div className="row g-4 justify-content-center">
            {/* ✅ अब यहाँ सॉर्टेड dbDrama का उपयोग हो रहा है */}
            {dbDrama.slice(0, 5).map((natak, index) => (
              <div className={`col-lg-2-4 col-md-4 col-6 ${index === 4 ? "mobile-hide-5th" : ""}`} key={natak._id || natak.id}>
                  ...
                  <div className="natak-square-card text-center border-0">
                    <div className="square-frame shadow-sm mb-3">
                      <img
                        ...
                        onError={(e) => { if (e.target.src !== window.location.origin + "/placeholder.jpg") e.target.src = "/placeholder.jpg"; }}
                      />
                    </div>
                    <h6 className="natak-title-v2 text-truncate px-1 text-dark">{natak.title}</h6>
                  </div>
                </Link>
              </div>
            ))}
          ...
          <div className="text-center mt-5">
            <Link to="/sahitya-sangrah/natak"><button className="btn-global-sangrah">समस्त संग्रह</button></Link>
          </div>
        </div>
      </section>
