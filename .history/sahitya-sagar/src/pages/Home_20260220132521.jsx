...
const Home = ({
    ...
    const fetchData = async () => {
      try {
        ...
        if (storyRes.data && storyRes.data.length > 0) {
          // पूरी लिस्ट को स्टेट में रखें
          setDbStories(storyRes.data);

          // 🔍 सिर्फ वही कहानी ढूंढें जिसका isFeatured: true है
          const picked = storyRes.data.find((s) => s.isFeatured === true);

          if (picked) {
            setFeaturedStory(picked);
          } else {
            // अगर किसी को true नहीं किया, तो बैकअप के लिए पहली वाली दिखाएं
  ...
  useEffect(() => {
    // जब लेखकों या कहानियों का डेटा आ जाए, तब बटन्स को एक्टिवेट करें
    if (
      (favoriteAuthors.length > 0 || dbStories.length > 0) &&
      typeof handleAuthorScroll === "function"
    ) {
      const timer = setTimeout(() => {
        handleAuthorScroll();
      }, 300); // 300ms बेहतर है ताकि रेंडरिंग पक्की हो जाए

      return () => clearTimeout(timer); // मेमोरी क्लीनअप के लिए
    }
  }, [favoriteAuthors, dbStories, handleAuthorScroll]);

  // बाकी 6 कहानियों के लिए (Slider) - slice(1, 7) मतलब index 1 से 6 तक
  const sliderStories = dbStories
    .filter((story) => story._id !== featuredStory?._id)
    .slice(0, 6); // सिर्फ 6 कहानियाँ
  if (loading)
      ...
      <div className="row justify-content-center mb-5">
        <div className="col-12 col-xl-11">
          <div
            ...
            style={{ cursor: "pointer" }}
            onClick={() =>
              navigate(`/read/${featuredStory.authorId}/${featuredStory.title}`)
            }
          >
            <div className="row g-0">
              <div className="col-md-6 overflow-hidden featured-img-box">
                <img
                  /* यहाँ बदला गया है: covers फोल्डर का पाथ जोड़ा गया */
                  src={featuredStory.img ? `http://localhost:5000/covers/${featuredStory.img}` : "/placeholder.jpg"}
          ...
          >
            ...
            >
              <div className="small-card-img">
                <img
                  /* यहाँ भी बदला गया है: covers फोल्डर का पाथ */
                  src={story.img ? `http://localhost:5000/covers/${story.img}` : "/placeholder.jpg"}
