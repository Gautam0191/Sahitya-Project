          ...
          {allResults.map((item) => {
            ...
            return (
              <div className="col-6 col-md-4 col-lg-3 d-flex" key={item._id || item.id}>
                <div className="card border-0 shadow-sm rounded-4 bg-white w-100 d-flex flex-column p-3 text-center transition-card">
                  ...
                  {item.isAuthor ? (
                    ...
                    <Link to={`/author/${aId}`} className="text-decoration-none h-100 d-flex flex-column align-items-center justify-content-between">
                      ...
                      <div className="flex-grow-1">
                        <h5 className="fw-bold text-dark mb-1" style={{ fontSize: "1.25rem" }}>{item.name}</h5>
                        <p className="text-muted small">साहित्यकार</p>
                      </div>
                      <div className="mt-2 fw-bold" style={{ color: "#5a2a3a" }}>प्रोफाइल देखें</div>
                    </Link>
                  ) : (
                    /* ✅ रचना कार्ड */
                    <div className="h-100 d-flex flex-column justify-content-between">
                      <div>
                        <span className="text-wine fw-bold small mb-2 d-block" style={{ opacity: "0.8" }}>
                          {getHindiType(item.type)}
                        </span>
                        <h5 className="fw-bold text-dark mb-2" style={{ fontSize: "1.3rem", lineHeight: "1.3" }}>
                          {wTitle}
                        </h5>
                        <p className="text-muted mb-3" style={{ fontSize: "0.95rem" }}>{item.authorName}</p>
                      </div>
                      <div className="mt-auto">
                        <Link to={`/read/${aId}/${wTitle}`} className="text-decoration-none fw-bold pden-link">
                          पढ़ें
                        </Link>
                      </div>
                    </div>
