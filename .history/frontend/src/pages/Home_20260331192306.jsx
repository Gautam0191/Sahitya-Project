      ...
      .map((item) => (
        ...
        >
          <div className="poetry-card-v2 shadow-sm h-100">
            <div className="card-img-wrapper" style={{ minHeight: '150px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f9f9f9' }}>
              {item.img ? (
                <img
                  /* ✅ स्मार्ट पाथ लॉजिक: अगर डेटा में / है तो वही लेगा, वरना /covers/ जोड़ेगा */
                  src={item.img.startsWith('/') ? item.img : `/covers/${item.img.trim()}`}
                  alt={item.title}
                  className="img-fluid"
                  onError={(e) => {
                ...
                <span className="text-muted small text-center px-2">{item.title}</span>
              )}
            </div>
